...
-- Patients with COPD who are on triple therapy (LABA, ICS, LAMA) in last 6 months who have not had an exacerbation 
-- in the last year
-- (opp cat of no FEV or FEV1 > 50%)
...
--#latestCopdInLastYear
IF OBJECT_ID('tempdb..#latestCopdInLastYear') IS NOT NULL DROP TABLE #latestCopdInLastYear
CREATE TABLE #latestCopdInLastYear (PatID int, latestCopdInLastYearDate date, latestCopdInLastYearMin varchar(512), latestCopdInLastYearMax varchar(512), 
	latestCopdInLastYear varchar(512), latestCopdInLastYearType varchar(512));
insert into #latestCopdInLastYear
select s.PatID, latestCopdInLastYearDate, MIN(Rubric), MAX(Rubric), 
	case 
...
from SIR_ALL_Records s
	inner join (
		select PatID, MAX(EntryDate) as latestCopdInLastYearDate from SIR_ALL_Records
		where PatID in (select PatID from #latestCopdCode)
		and (
			(
	--Pred
		...
		group by PatID
	) sub on sub.PatID = s.PatID and sub.latestCopdInLastYearDate = s.EntryDate
where s.PatID in (select PatID from #latestCopdCode)
and (
	ReadCode in 
			(
...
and EntryDate < @refdate
and EntryDate > DATEADD(year, -1, @refdate)
group by s.PatID, latestCopdInLastYearDate
...
-- 		inner join #latestCopdInLastYear as t on t.PatID = r.PatID
-- 		where r.PatID in (select PatID from #latestCopdCode)
-- 		and ReadCode in (select code from codeGroups where [group] = 'pulRehabOfferedSs')
-- 		and EntryDate < @refdate
-- 		and EntryDate >= latestCopdInLastYearDate --so if there is an exac code AND pul rehab code on the same day, pul rehab wins
-- 		group by r.PatID
-- 	) sub on sub.PatID = s.PatID and sub.firstPulRehabCodeAfterExacDate = s.EntryDate
-- inner join #latestCopdInLastYear as t on t.PatID = s.PatID
-- where s.PatID in (select PatID from #latestCopdCode)
-- and ReadCode in (select code from codeGroups where [group] = 'pulRehabOfferedSs')
-- and EntryDate < @refdate
-- and EntryDate >= latestCopdInLastYearDate --so if there is an exac code AND pul rehab code on the same day, pul rehab wins
...
-- TODO triple therapy patients
IF OBJECT_ID('tempdb..#tripleTherapyLast6Months') IS NOT NULL DROP TABLE #tripleTherapyLast6Months
CREATE TABLE #tripleTherapyLast6Months 
	(); --TODO
insert into #tripleTherapyLast6Months
select a.PatID

-- TODO codes
-- LABA,ICS,LABA+ICS from SMASH
-- LAMA
-- c33..00 TIOTROPIUM      c3...
-- c331.00 TIOTROPIUM 18mcg inhal caps     c33..
-- c331.00 TIOTROPIUM 18micrograms inhalation capsules     c33..
-- c332.00 TIOTROPIUM 18mcg caps+device    c33..
-- c332.00 TIOTROPIUM 18micrograms capsules with inhaler device    c33..
-- c333.00 TIOTROPIUM 2.5mcg carts+device  c33..
-- c333.00 TIOTROPIUM 2.5micrograms inhalation carts+inhaler device        c33..
-- c333.00 TIOTROPIUM 2.5micrograms inhalation cartridges with inhaler device      c33..
-- c33x.00 SPIRIVA RESPIMAT 2.5mcg carts   c33..
-- c33x.00 SPIRIVA RESPIMAT 2.5mcg cartridges+Respimat inhaler device      c33..
-- c33x.00 SPIRIVA RESPIMAT 2.5micrograms inhalation cartridges with Respimat inhaler device       c33..
-- c33y.00 SPIRIVA COMBOPACK 18mcg caps    c33..
-- c33y.00 SPIRIVA COMBOPACK 18mcg caps+HandiHaler inhaler device  c33..
-- c33y.00 SPIRIVA COMBOPACK 18micrograms capsules with HandiHaler inhaler device  c33..
-- c33z.00 SPIRIVA 18mcg inhalation caps   c33..
-- c33z.00 SPIRIVA 18micrograms inhalation capsules        c33..
-- c34..00 ACLIDINIUM      c3...
-- c341.00 EKLIRA GENUAIR 322mcg/dose inh  c34..
-- c341.00 EKLIRA GENUAIR 322micrograms/dose dry powder inhaler    c34..
-- c342.00 ACLIDINIUM 322mcg/dose inhaler  c34..
-- c342.00 ACLIDINIUM 322micrograms/dose dry powder inhaler        c34..
-- c35..00 UMECLIDINIUM    c3...
-- c351.00 INCRUSE ELLIPTA 55mcg/dose inh  c35..
-- c351.00 INCRUSE ELLIPTA 55micrograms/dose dry powder inhaler    c35..
-- c352.00 UMECLIDINIUM 55mcg/dose inh     c35..
-- c352.00 UMECLIDINIUM 55micrograms/dose dry powder inhaler       c35..
...
select a.PatID,
	case when
		...
		and (latestCopdInLastYearDate is not null and latestCopdInLastYearDate > DATEADD(month, -12, @achievedate)) --exacerbation in the current achievement year
		and --no exclusions
...
from #latestCopdCode as a
	...
	left outer join (select PatID, latestCopdInLastYearDate from #latestCopdInLastYear) e on e.PatID = a.PatID
...
select a.PatID,
	case when 
		denominator = 1
		and (firstPulRehabCodeAfterExacDate is not null and firstPulRehabCodeAfterExacDate >=  DATEADD(month, -2, latestCopdInLastYearDate))
		then 1 
		else 0 
		end as numerator
from #latestCopdCode as a
		...
		left outer join (select PatID, latestCopdInLastYearDate from #latestCopdInLastYear) d on d.PatID = a.PatID
...
CREATE TABLE #eligiblePopulationAllData (PatID int, 
	...
	latestCopdInLastYear varchar(512), latestCopdInLastYearDate date, latestCopdInLastYearType varchar(512),
...
select a.PatID, 
	...
	latestCopdInLastYear, latestCopdInLastYearDate, latestCopdInLastYearType,
...
from #latestCopdCode as a
		...
		left outer join (select PatID, latestCopdInLastYear, latestCopdInLastYearDate, latestCopdInLastYearType from #latestCopdInLastYear) m on m.PatID = a.PatID
...
select a.PatID, 'copd.exacerbation.rehab',
		...
		'<li>Latest COPD exacerbation was on ' + CONVERT(VARCHAR, latestCopdInLastYearDate, 3) + '.' +
			case 
				when latestCopdInLastYearType = 'tabs' then ' (This may not be coded in the record, and may only appear as a prescription of antibiotics and/or steroids).'
				else ''
			end + 
		'</li>'+
...
select s.PatID, latestUncodedPulRehabCodeBeforeExacDate, MAX(Rubric) as latestUncodedPulRehabCodeBeforeExac from SIR_ALL_Records as s
	...
	(
		...
		and EntryDate < @refdate
		and EntryDate < latestCopdInLastYearDate
		group by r.PatID
...
select s.PatID, firstUncodedPulRehabCodeAfterExacDate, MAX(Rubric) as firstUncodedPulRehabCodeAfterExac from SIR_ALL_Records as s
	...
	(
		...
		and ReadCode in (select code from codeGroups where [group] = 'pulRehabUncoded')
		and EntryDate < @refdate
		and EntryDate >= latestCopdInLastYearDate --so if there is an exac code AND pul rehab code on the same day, it = pul rehab
		group by r.PatID
...
from #impOppsData as a
	left outer join (select PatID, latestCopdInLastYear, latestCopdInLastYearDate, firstCopdCodeDate, latestCopdInLastYearType, latestMrc2CodeDate from #eligiblePopulationAllData) as b on b.PatID = a.PatID
where latestCopdInLastYearDate > DATEADD(MONTH, -2, @refdate)
union
...
select a.PatID,
	...
	as supportingText
from #impOppsData as a
	left outer join (select PatID, firstPulRehabCodeAfterExacDate, latestCopdInLastYear, latestCopdInLastYearType, latestCopdInLastYearDate, firstCopdCodeDate, latestMrc2CodeDate from #eligiblePopulationAllData) as b on b.PatID = a.PatID
where latestCopdInLastYearDate <= DATEADD(MONTH, -2, @refdate)
...
select a.PatID,
	...
	@ptPercPoints as pointsPerAction,
	1 as priority,
	'Record ''COPD exacerbation'' (using code H3122 [H3122]) on ' + CONVERT(VARCHAR, latestCopdInLastYearDate, 3) + '.' as actionText,
	'Reasoning'+
		'<ul>'+
		'<li>Patient has COPD (since ' + CONVERT(VARCHAR, firstCopdCodeDate, 3) + ').</li>'+
		'<li>Latest COPD exacerbation was on ' + CONVERT(VARCHAR, latestCopdInLastYearDate, 3) + '.</li>' +
...
from #impOppsData as a
	left outer join (select PatID, firstPulRehabCodeAfterExacDate, latestCopdInLastYear, latestCopdInLastYearType, latestCopdInLastYearDate, firstCopdCodeDate, latestMrc2CodeDate from #eligiblePopulationAllData) as b on b.PatID = a.PatID
where latestCopdInLastYearType = 'tabs'
union
...
select a.PatID,
	...
	'Suggested action'+
		...
		'<li>Record ''Pulmonary rehabilitation offered'' using code 9NSL. [9NSL.] on date ' +
			case
				when 
					firstUncodedPulRehabCodeAfterExacDate <= DATEADD(MONTH, 2, latestCopdInLastYearDate) 
					and latestUncodedPulRehabCodeBeforeExacDate < DATEADD(WEEK, -6, latestCopdInLastYearDate)
				then CONVERT(VARCHAR, firstUncodedPulRehabCodeAfterExacDate, 3) --less than or equal to 2/12 beyond latest exac code
				when 
					latestUncodedPulRehabCodeBeforeExacDate >= DATEADD(WEEK, -6, latestCopdInLastYearDate) 
					and firstUncodedPulRehabCodeAfterExacDate > DATEADD(MONTH, 2, latestCopdInLastYearDate) 
	...
	as supportingText
from #impOppsData as a
	left outer join (select PatID, latestCopdInLastYear, latestCopdInLastYearDate, latestCopdInLastYearType, firstCopdCodeDate, latestMrc2CodeDate from #eligiblePopulationAllData) as b on b.PatID = a.PatID
where
	(firstUncodedPulRehabCodeAfterExacDate <= DATEADD(MONTH, 2, latestCopdInLastYearDate))
	or (latestUncodedPulRehabCodeBeforeExacDate > DATEADD(WEEK, -6, latestCopdInLastYearDate))
union
...
--		'<li><a href="http://www.salfordccg.nhs.uk/respiratory-disease#key" target="_blank" title="Salford Standards">Salford Standards</a> and <a href="https://cks.nice.org.uk/chronic-obstructive-pulmonary-disease#!scenariorecommendation:2" target="_blank" title="NICE Clinical Knowledge Summary">NICE guidelines</a> recommend these patients are offered pulmonary rehabilitation < 2 months afterwards because <a href="https://www.ncbi.nlm.nih.gov/pmc/articles/PMC1164434/" target="_blank" title="Respiratory Research Journal">evidence suggests it may decrease hospital admission and mortality risk, and increase exercise capacity and quality of life</a>.</li>'+
	...
	as supportingText
from #impOppsData as a
	left outer join (select PatID, latestCopdCodeDate, latestCopdInLastYear, latestCopdInLastYearType, latestMrc2CodeDate, firstCopdCodeDate, latestCopdInLastYearDate from #eligiblePopulationAllData) as c on c.PatID = a.PatID
where
	((latestPalCodeDate > DATEADD(year, -1, @refdate)) and (latestPalPermExCodeDate is null or latestPalPermExCodeDate < latestPalCodeDate))
...
select c.pracID, 'missedRehab', 
	SUM(case when denominator = 1 and numerator = 0 and latestCopdInLastYearDate <= DATEADD(MONTH, -2, @refdate) then 1.0 else 0.0 end)
	/
	SUM(case when denominator = 1 then 1.0 else 0.0 end),
	SUM(case when denominator = 1 and numerator = 0 and latestCopdInLastYearDate <= DATEADD(MONTH, -2, @refdate) then 1.0 else 0.0 end),	
	SUM(case when denominator = 1 and numerator = 0 and latestCopdInLastYearDate <= DATEADD(MONTH, -2, @refdate)  then 1.0 else 0.0 end)*@ptPercPoints
from #eligiblePopulationAllData as a
...
select c.pracID, 'uncodedExacerbations', 
	SUM(case when latestCopdCode is not null and latestCopdInLastYearType = 'tabs' then 1.0 else 0.0 end)
	/
	SUM(case when latestCopdCode is not null and latestCopdInLastYear is not null then 1.0 else 0.0 end),
	SUM(case when latestCopdCode is not null and latestCopdInLastYearType = 'tabs' then 1.0 else 0.0 end),
	SUM(case when latestCopdCode is not null and latestCopdInLastYearType = 'tabs' then 1.0 else 0.0 end)*@ptPercPoints
from #eligiblePopulationAllData as a
left outer join ptPractice as c on c.PatID = a.PatID
group by c.pracID
having SUM(case when denominator = 1 then 1.0 else 0.0 end) > 0 --where denom is not 0
