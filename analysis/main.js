...
const extractIndicatorJsons = () => {
  ...
  tars.forEach((t) => {
    ...
    fs.createReadStream(path.join(TAR_DIR, t)).pipe(tar.extract(INDICATOR_DIR, {
      ignore: name => name.indexOf('indicators.json') < 0,
      map: (header) => {
...
const getData = () => {
  ...
  const oo = fs.createWriteStream(path.join(__dirname, 'data', 'output.txt'));
  const todo = indicators.length;
  let done = 0;
  indicators.forEach((i) => {
    const ts = i.split('.')[0];

    const s = fs.createReadStream(path.join(INDICATOR_DIR, i));
    s.pipe(es.split()).pipe(es
      .mapSync((line) => {
...
const getTrendData = () => {
  ...
  if (mostRecentIndicator) {
    const s = fs.createReadStream(path.join(INDICATOR_DIR, mostRecentIndicator));
    ...
    s.pipe(es.split()).pipe(es
      .mapSync((line) => {
        ...
        // process line here and call s.resume() when rdy
        // function below was for logging memory usage
        const deets = getTrendDeets(line);
        if (deets) {
          oo.write(deets);
          oo.write('\n');
        }
