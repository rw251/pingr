...
IF EXISTS(SELECT * FROM sys.objects WHERE Type = 'P' AND Name ='pingr.ckdAndProt.treatment.bp') DROP PROCEDURE [pingr.ckdAndProt.treatment.bp];
GO
CREATE PROCEDURE [pingr.ckdAndProt.treatment.bp] @refdate VARCHAR(10), @JustTheIndicatorNumbersPlease bit = 0
AS
SET NOCOUNT ON
...
set @achieveDate = (select 
case --for 6 monthly targets
	when MONTH(@refdate) <4 then CONVERT(VARCHAR,YEAR(@refdate)) + '-03-31' --when today's date is before April, refdate is 31st March this year
	when MONTH(@refdate) >3 and MONTH(@refdate) <10 then CONVERT(VARCHAR,(YEAR(@refdate))) + '-09-30' --when today's date is after March BUT before October, refdate is 30th September this year
	when MONTH(@refdate) >9 then CONVERT(VARCHAR,(YEAR(@refdate) + 1)) + '-03-31' --when today's date is after September, refdate is 31st March NEXT year
end); --31st March
...
select a.PatID, '&lt;130/80' as bpTarget, --SS
	case when (latestSbpDate > DATEADD(month, -6, @achievedate)) and (latestDbpDate > DATEADD(month, -6, @achievedate)) then 1 else 0 end as bpMeasuredOk,  --measured within 12/12 of achievedate (SS)
	case when latestSbp < 130 and latestDbp < 80 then 1 else 0 end as bpControlledOk
from #firstCkd35code as a
...
CREATE TABLE #exclusions
	(PatID int, noProtExclude int, ageExclude int, regCodeExclude int, deRegCodeExclude int, tempExExclude int,
...
from #latestCkd35code as a
	...
	left outer join (select PatID, protPatient from #dmProtPatient) k on k.PatID = a.PatID
...
select a.PatID,
	case when noProtExclude = 0 and ageExclude = 0 and permExExclude  = 0 and tempExExclude  = 0 and regCodeExclude  = 0
		and diagExclude  = 0 and deRegCodeExclude  = 0 and 	deadCodeExclude  = 0 and deadTableExclude  = 0
		then 1 else 0 end as denominator
from #latestCkd35code as a
	left outer join (select PatID, noProtExclude, ageExclude, permExExclude, tempExExclude, regCodeExclude, diagExclude,
...
union
select 'ckdAndProt.treatment.bp', b.pracID, CONVERT(char(10), @refdate, 126) as date, sum(case when numerator = 1 then 1 else 0 end) as numerator, sum(case when denominator = 1 then 1 else 0 end) as denominator, 0.80 as target, @abc from #eligiblePopulationAllData as a
	inner join ptPractice as b on a.PatID = b.PatID
	group by b.pracID;
...
select PatID, 'ckdAndProt.treatment.bp',
	case
		...
		when bpMeasuredOK = 1 and bpControlledOk = 0 then
			'<ul><li>Patient is on CKD register and their last ACR was greater than 70 on ' + CONVERT(VARCHAR, latestAcrDate, 3) +'.' +
				case 
					when sourceAcr = 'salfordt' then '(This ACR was taken in <strong>hospital</strong> so may not appear in the GP record)'
				else ''
				end +			
			'</li>
			<li>Target BP is <a href=''http://cks.nice.org.uk/chronic-kidney-disease-not-diabetic#!scenariorecommendation:5'' target=''_blank'' title="NICE BP">&lt;130/80 mmHg</a>.</li>
		...
		when numerator = 1 then
			'<ul><li>Patient is on CKD register and their last ACR was greater than 70 on ' + CONVERT(VARCHAR, latestAcrDate, 3) +'.</li>
...
--CHECK REGISTERED
select a.PatID,
	'ckdAndProt.treatment.bp' as indicatorId,
...
select c.pracID, 'measureActions', 
	SUM(case when indicatorId = 'ckdAndProt.treatment.bp' and actionCat = 'Measure BP' then 1.0 else 0.0 end)
	/
	SUM(case when denominator = 1 then 1.0 else 0.0 end),
	SUM(case when indicatorId = 'ckdAndProt.treatment.bp' and actionCat = 'Measure BP' then 1.0 else 0.0 end),
	SUM(case when indicatorId = 'ckdAndProt.treatment.bp' and actionCat = 'Measure BP' then 1.0 else 0.0 end)*@ptPercPoints
from #eligiblePopulationAllData as a
...
values
--overview tab
('ckdAndProt.treatment.bp','name','CKD and ACR &ge; 70mg/mmol blood pressure control'), --overview table name
('ckdAndProt.treatment.bp','tabText','CKD + ACR BP Control'), --indicator tab text
('ckdAndProt.treatment.bp','description', --'show more' on overview tab
	'<strong>Definition:</strong>Patients on the CKD register with ACR 70mg/mmol or more with a BP recorded in the last 6 months (since ' +
		case
			when MONTH(@refdate) <4 then '1st October ' + CONVERT(VARCHAR,(YEAR(@refdate) - 1)) --when today's date is before April, it's 1st October LAST year
			when MONTH(@refdate) >3 and MONTH(@refdate) <10 then '1st April ' + CONVERT(VARCHAR,(YEAR(@refdate))) --when today's date is after March BUT before October, it's 1st April THIS year
			when MONTH(@refdate) >9 then '1st October ' + CONVERT(VARCHAR,(YEAR(@refdate))) --when today's date is after September, it's 1st October THIS year
		end +
	') where the latest BP is &lt;130/80 mmHg.<br>' + 
...
--indicator tab
--summary text
('ckdAndProt.treatment.bp','tagline','of your patients on the CKD register with ACR 70mg/mmol or more have had a BP measurement in the 6 months (from ' +
		case
			when MONTH(@refdate) <4 then '1st October ' + CONVERT(VARCHAR,(YEAR(@refdate) - 1)) --when today's date is before April, it's 1st October LAST year
			when MONTH(@refdate) >3 and MONTH(@refdate) <10 then '1st April ' + CONVERT(VARCHAR,(YEAR(@refdate))) --when today's date is after March BUT before October, it's 1st April THIS year
			when MONTH(@refdate) >9 then '1st October ' + CONVERT(VARCHAR,(YEAR(@refdate))) --when today's date is after September, it's 1st October THIS year
...
--registered?
('ckdAndProt.treatment.bp','opportunities.Registered?.name','Check registered'),
('ckdAndProt.treatment.bp','opportunities.Registered?.description','Patients who have not had contact with your practice in the last 12 months - are they still registered with you?'),
('ckdAndProt.treatment.bp','opportunities.Registered?.positionInBarChart','3'),

--Measure BP
('ckdAndProt.treatment.bp','opportunities.Measure BP.name','Measure BP'),
('ckdAndProt.treatment.bp','opportunities.Measure BP.description','Patients who may achieve the indicator by simply re-measuring their BP i.e. those without a BP reading, or with a one-off high reading, recent medication optimisation, or only slightly uncontrolled BP.'),
('ckdAndProt.treatment.bp','opportunities.Measure BP.positionInBarChart','1'),

--MEDICATION SUGGESTION
('ckdAndProt.treatment.bp','opportunities.Medication optimisation.name','Medication suggestion'),
('ckdAndProt.treatment.bp','opportunities.Medication optimisation.description','Patients with uncontrolled BP who are prescribed suboptimal medication.'),
('ckdAndProt.treatment.bp','opportunities.Medication optimisation.positionInBarChart',	'2'),

--SUGGEST EXCLUDE
('ckdAndProt.treatment.bp','opportunities.Suggest exclude.name','Suggest exclude'),
('ckdAndProt.treatment.bp','opportunities.Suggest exclude.description','Patients who may benefit from being excluded from this quality indicators.'),
('ckdAndProt.treatment.bp','opportunities.Suggest exclude.positionInBarChart',	'4')
