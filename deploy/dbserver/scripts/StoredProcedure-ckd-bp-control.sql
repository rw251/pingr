...
select a.PatID, '&lt;140/90' as bpTarget, --SS
	...
	case when latestSbp < 140 and latestDbp < 90 then 1 else 0 end as bpControlledOk
from #firstCkd35code as a
...
--CCG view
select 'ckd.treatment.bp', 'ALL', CONVERT(char(10), @refdate, 126) as date, sum(case when numerator = 1 then 1 else 0 end) as numerator, sum(case when denominator = 1 then 1 else 0 end) as denominator, @target, @abc from #eligiblePopulationAllData as a
union
--Individual practice view
select 'ckd.treatment.bp', b.pracID, CONVERT(char(10), @refdate, 126) as date, sum(case when numerator = 1 then 1 else 0 end) as numerator, sum(case when denominator = 1 then 1 else 0 end) as denominator, @target, @abc from #eligiblePopulationAllData as a
	inner join ptPractice as b on a.PatID = b.PatID
	group by b.pracID;
...
select PatID, 'ckd.treatment.bp',
	case
		when bpMeasuredOK = 0 then 
			...
			<li>Salford Standards recommend BP should be measured in the 6 months since ' +
				case
		...
		when bpMeasuredOK = 1 and bpControlledOk = 0 then
			'<ul><li>Patient is on CKD register.</li>
			<li>Target BP is <a href=''http://cks.nice.org.uk/chronic-kidney-disease-not-diabetic#!scenariorecommendation:5'' target=''_blank'' title="NICE BP targets in CKD">&lt;140/90 mmHg</a>.</li>
			<li>Last BP was <strong>uncontrolled</strong>: ' +
					case
						when latestSbp >= 140 then '<strong>' + Str(latestSbp) + '</strong>'
						else Str(latestSbp)
					end
				+ '/' +
					case
		...
		when numerator = 1 then
			...
			<li>This is in accordance with both the Salford Standards and <a href=''http://cks.nice.org.uk/chronic-kidney-disease-not-diabetic#!scenariorecommendation:5'' target=''_blank'' title="NICE BP targets in CKD">NICE guidelines</a>.</li>' +
			case 
				when sourceSbp = 'salfordt' then '<li>This reading was taken in <strong>hospital</strong> so may not appear in the GP record.</li></ul>'
			else '</ul>'
			end			
...
from #medSuggestion as a
	left outer join (select PatID, latestSbp, latestDbp, latestSbpDate, sourceSbp, bpTarget, dmPatient, protPatient, latestAcrDate from #eligiblePopulationAllData) as b on b.PatID = a.PatID
...
('ckd.treatment.bp','tabText','CKD BP Control'), --indicator tab text
('ckd.treatment.bp','description', --'show more' on overview tab
	'<strong>Definition:</strong>Patients on the CKD register with a BP recorded in the last 6 months (since ' +
		case
			when MONTH(@refdate) <4 then '1st October ' + CONVERT(VARCHAR,(YEAR(@refdate) - 1)) --when today's date is before April, it's 1st October LAST year
			when MONTH(@refdate) >3 and MONTH(@refdate) <10 then '1st April ' + CONVERT(VARCHAR,(YEAR(@refdate))) --when today's date is after March BUT before October, it's 1st April THIS year
			when MONTH(@refdate) >9 then '1st October ' + CONVERT(VARCHAR,(YEAR(@refdate))) --when today's date is after September, it's 1st October THIS year
		end +
	') where the latest BP is &lt;140/90 mmHg.<br>' +
...
--indicator tab
--summary text
('ckd.treatment.bp','tagline','of your patients on the CKD register have had a BP measurement in the 6 months (from ' +
		case											
			when MONTH(@refdate) <4 then '1st October ' + CONVERT(VARCHAR,(YEAR(@refdate) - 1)) --when today's date is before April, it's 1st October LAST year
			when MONTH(@refdate) >3 and MONTH(@refdate) <10 then '1st April ' + CONVERT(VARCHAR,(YEAR(@refdate))) --when today's date is after March BUT before October, it's 1st April THIS year
			when MONTH(@refdate) >9 then '1st October ' + CONVERT(VARCHAR,(YEAR(@refdate))) --when today's date is after September, it's 1st October THIS year
...
('ckd.treatment.bp','positiveMessage', --tailored text
	case 
