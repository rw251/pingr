...
var iap = {
  ...
  updateAction: function(action) {
    ...
    actionContainer = $('#individualActionCard');

    var yesbox = actionContainer.find('li[data-id="' + action.actionTextId + '"] label.btn-yes input');
    var nobox = actionContainer.find('li[data-id="' + action.actionTextId + '"] label.btn-no input');
  ...
  wireUp: function(pathwayId, pathwayStage, standard, patientId) {
    //actionPanel = $('');
    //userActionPanel = $('#left-panel');

    actionPanel = $('#tab-plan-addition');
    userActionPanel = $('#left-panel');

    actionCard = $('#individualActionCard');
    suggestionCard = $('#individualSuggestionCard');
    suggestionListCard = $('#individualSuggestionListCard');
  ...
  populateIndividualSuggestedActions: function(patientId, pathwayId, pathwayStage, standard, visible) {
    ...
    $('#advice-list').find('td:contains("Reasoning")').each(function() {
      ...
      if (i < contents.length - 1) {
        ...
        reasoning.replaceWith('Reasoning <button type="button" data-clipboard-text="' + content + '" data-content="Copied!<br><strong>Use Ctrl + v to paste into ' + state.selectedPractice.ehr + '!</strong>" data-toggle="tooltip" data-placement="top" title="Copy reasoning to clipboard." class="btn btn-xs btn-default btn-copy"><span class="material-icons">content_copy</span></button>');
      }
    });
