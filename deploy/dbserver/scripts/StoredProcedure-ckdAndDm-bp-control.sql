...
IF EXISTS(SELECT * FROM sys.objects WHERE Type = 'P' AND Name ='pingr.ckdAndDm.treatment.bp') DROP PROCEDURE [pingr.ckdAndDm.treatment.bp];
GO
CREATE PROCEDURE [pingr.ckdAndDm.treatment.bp] @refdate VARCHAR(10), @JustTheIndicatorNumbersPlease bit = 0
AS
SET NOCOUNT ON
...
CREATE TABLE #exclusions
	(PatID int, noDmExclude int, ageExclude int, regCodeExclude int, deRegCodeExclude int, tempExExclude int,
	deadCodeExclude int, deadTableExclude int, diagExclude int, permExExclude int);
insert into #exclusions
select a.PatID,
	case when dmPatient = 0 then 1 else 0 end as noDmExclude, -- Demographic exclusions: Under 18 at achievement date (from QOF v34 business rules)
...
from #latestCkd35code as a
	...
	left outer join (select PatID, dmPatient from #dmProtPatient) k on k.PatID = a.PatID
...
select a.PatID,
	case when noDmExclude = 0 and ageExclude = 0 and permExExclude  = 0 and tempExExclude  = 0 and regCodeExclude  = 0
		and diagExclude  = 0 and deRegCodeExclude  = 0 and 	deadCodeExclude  = 0 and deadTableExclude  = 0
		then 1 else 0 end as denominator
from #latestCkd35code as a
	left outer join (select PatID, noDmExclude, ageExclude, permExExclude, tempExExclude, regCodeExclude, diagExclude,
...
select a.PatID, 'ckdAndDm.treatment.bp',
	case
...
select c.pracID, 'measureActions', 
	SUM(case when indicatorId = 'ckdAndDm.treatment.bp' and actionCat = 'Measure BP' then 1.0 else 0.0 end)
	/
	SUM(case when denominator = 1 then 1.0 else 0.0 end),
	SUM(case when indicatorId = 'ckdAndDm.treatment.bp' and actionCat = 'Measure BP' then 1.0 else 0.0 end),
	SUM(case when indicatorId = 'ckdAndDm.treatment.bp' and actionCat = 'Measure BP' then 1.0 else 0.0 end)*@ptPercPoints
from #eligiblePopulationAllData as a
...
values
--overview tab
('ckdAndDm.treatment.bp','name','CKD and DM blood pressure control'), --overview table name
('ckdAndDm.treatment.bp','tabText','CKD + DM BP control'), --indicator tab text
...
('ckdAndDm.treatment.bp','positiveMessage', --tailored text
	case 
...
--registered?
('ckdAndDm.treatment.bp','opportunities.Registered?.name','Check registered'),
('ckdAndDm.treatment.bp','opportunities.Registered?.description','Patients who have not had contact with your practice in the last 12 months - are they still registered with you?'),
('ckdAndDm.treatment.bp','opportunities.Registered?.positionInBarChart','3'),

--Measure BP
('ckdAndDm.treatment.bp','opportunities.Measure BP.name','Measure BP'),
('ckdAndDm.treatment.bp','opportunities.Measure BP.description','Patients who may achieve the indicator by simply re-measuring their BP i.e. those without a BP reading, or with a one-off high reading, recent medication optimisation, or only slightly uncontrolled BP.'),
('ckdAndDm.treatment.bp','opportunities.Measure BP.positionInBarChart','1'),

--MEDICATION SUGGESTION
('ckdAndDm.treatment.bp','opportunities.Medication optimisation.name','Medication suggestion'),
('ckdAndDm.treatment.bp','opportunities.Medication optimisation.description','Patients with uncontrolled BP who are prescribed suboptimal medication.'),
('ckdAndDm.treatment.bp','opportunities.Medication optimisation.positionInBarChart',	'2'),

--SUGGEST EXCLUDE
('ckdAndDm.treatment.bp','opportunities.Suggest exclude.name','Suggest exclude'),
('ckdAndDm.treatment.bp','opportunities.Suggest exclude.description','Patients who may benefit from being excluded from this quality indicators.'),
('ckdAndDm.treatment.bp','opportunities.Suggest exclude.positionInBarChart',	'4')
