----------------------------------------------------------------------------------------------
--CKD CORRECT CODING - v3.0 - 6/7/16
----------------------------------------------------------------------------------------------
...
--Text file:
----------------------------------------------------------------------------------------------
--name	Correct CKD diagnosis coding
--description	<strong>Description:</strong> Patients with correct coding of CKD (Stage 3 or above) based on their <a target='_blank' href='http://cks.nice.org.uk/chronic-kidney-disease-not-diabetic#!diagnosissub:2'>eGFR and ACR readings</a>. <br> <strong>Eligible patients:</strong> Patients with a CKD diagnostic code (stage 3 or above) without an exclusion code. <br> <strong>Correct patients:</strong> Patients with a CKD code matching their latest eGFR and/or ACR readings.
--tagline	of your patients with a CKD code have the correct stage recorded based on their <a href="http://cks.nice.org.uk/chronic-kidney-disease-not-diabetic#!diagnosissub:2" target="_blank">eGFR and ACR readings</a>.
...
--opportunities.wrongStage.name	Wrong stage
--opportunities.wrongStage.description	Patients who have the <strong>wrong CKD stage</strong> in their records according to their latest eGFR and ACR readings.
--opportunities.rightStageWrongSubstage.name	Right stage, wrong substage
--opportunities.rightStageWrongSubstage.description	Patients who have the <strong>right CKD stage</strong> but the <strong>wrong CKD substage</strong> in their records according to their latest eGFR and ACR readings.
--opportunities.overdiagnosed.name	Overdiagnosed
--opportunities.overdiagnosed.description	Patients with a CKD code whose <strong>latest eGFR readings do not meet the criteria for CKD diagnosis stage 3 or above</strong>
--opportunities.acrTest.name	ACR missing
--opportunities.acrTest.description	Patients with a CKD code who do not have an ACR test result in their record to enable accurate <a href="http://cks.nice.org.uk/chronic-kidney-disease-not-diabetic#!diagnosissub:2" target="_blank">CKD staging</a>.
...
--------------------------------------------------------------------------------------------------
--27/5/16
--Wrong stage - ACR unknown - should we still suggest a code for these patients? Or just recommend to get an ACR before recommending one?
--Right stage, wrong substage (ACR always known) 
	--want as separate indicator?
	--wrong substage includes NO substage (e.g. 3a) - that OK?
	--3a/b counts as a substage - that ok?
--ACR test needed includes where 'Right stage, but ACR unknown' e.g. where code in records says there is proteinuria, but there's no ACR in the record - that OK?
--Includes overdiagnosed - want that here or in undiagnosed?
...
select 'ckd.diagnosis.staging',b.pracID, CONVERT(char(10), @refdate, 126) as date, sum(case when indicator='right' then 1 else 0 end) as numerator, SUM(case when code is not null then 1 else 0 end) as denominator, 0.75 as target from #indicator as a
...
--export data files
--------------------------------------------------------------------------------
--EXEC xp_cmdshell 'bcp "select * from P8701617062016.dbo.outContacts" queryout "C:\Users\MPHStudent\Desktop\pingr\data\in\ckd.diagnosis.staging\contacts.txt" -T -c'
--EXEC xp_cmdshell 'bcp "select * from P8701617062016.dbo.outImpCodes" queryout "C:\Users\MPHStudent\Desktop\pingr\data\in\ckd.diagnosis.staging\important codes.txt" -T -c'
--EXEC xp_cmdshell 'bcp "select * from P8701617062016.dbo.outDiagnoses" queryout "C:\Users\MPHStudent\Desktop\pingr\data\in\ckd.diagnosis.staging\diagnoses.txt" -T -c'
--EXEC xp_cmdshell 'bcp "select * from P8701617062016.dbo.outMeasures" queryout "C:\Users\MPHStudent\Desktop\pingr\data\in\ckd.diagnosis.staging\measures.txt" -T -c'
--EXEC xp_cmdshell 'bcp "select * from P8701617062016.dbo.outDemographics" queryout "C:\Users\MPHStudent\Desktop\pingr\data\in\ckd.diagnosis.staging\demographics.txt" -T -c'
--EXEC xp_cmdshell 'bcp "select * from P8701617062016.dbo.outImpOppCatsAndActions" queryout "C:\Users\MPHStudent\Desktop\pingr\data\in\ckd.diagnosis.staging\impOppCatsAndActions.txt" -T -c'
--EXEC xp_cmdshell 'bcp "select * from P8701617062016.dbo.outIndicator" queryout "C:\Users\MPHStudent\Desktop\pingr\data\in\ckd.diagnosis.staging\indicator.txt" -T -c'
